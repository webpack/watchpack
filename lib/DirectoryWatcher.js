...
function DirectoryWatcher(directoryPath, options) {
	...
	var events = options.events || ["add", "addDir", "change", "unlink", "unlinkDir"];
	var eventsIgnored = options.eventsIgnored || [];
	this.watchEvents = events.filter(function(event) {
		return eventsIgnored.indexOf(event) === -1;
	});
	var eventHandlers = {
		"add": this.onFileAdded.bind(this),
		"addDir": this.onDirectoryAdded.bind(this),
		"change": this.onChange.bind(this),
		"unlink": this.onFileUnlinked.bind(this),
		"unlinkDir": this.onDirectoryUnlinked.bind(this),
	};
	var watcher = this.watcher
	this.watchEvents.forEach(function (event) {
		watcher.on(event, eventHandlers[event]);
	});
