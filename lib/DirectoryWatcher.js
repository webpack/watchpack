...
let { FS_ACCURACY, ERR } = require("./constants");
...
class DirectoryWatcher extends EventEmitter {
		...
		if (this._activeEvents.get(filename) === undefined) {
			...
			const checkStats = () => {
				...
				fs.stat(filePath, (err, stats) => {
					...
					if (err) {
						if (err.code !== ERR.ENOENT && err.code !== ERR.EPERM && err.code !== ERR.EBUSY) {
	...
	onWatcherError(err) {
		if (this.closed) return;
		if (err) {
			if (err.code !== ERR.EPERM && err.code !== ERR.ENOENT) {
	...
	doScan(initial) {
		...
		fs.readdir(this.path, (err, items) => {
			if (this.closed) return;
			if (err) {
				if (err.code === ERR.ENOENT || err.code === ERR.EPERM) {
					this.watchInParentDirectory();
				} else {
			...
			async.forEach(
				...
				(itemPath, callback) => {
					fs.stat(itemPath, (err2, stats) => {
						if (this.closed) return;
						if (err2) {
							if (err2.code === ERR.ENOENT || err2.code === ERR.EPERM || err2.code === ERR.EBUSY) {
						...
						if (stats.isFile()) {
							...
							if (!initial || !this.directories.has(itemPath)) this.setDirectory(itemPath, +stats.mtime || +stats.ctime || 1, initial, "scan (dir)");
						}
						callback();
					});
				},
