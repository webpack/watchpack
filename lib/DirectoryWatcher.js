		...
		process.nextTick(() => {
			...
			fs.readdir(this.path, (err, items) => {
				...
				if (err) {
					...
					// invalid-argument errors on the directory itself as removed
					// rather than logging "Watchpack Error (initial scan)". These
					// surface for unreadable mounts (WSL `/mnt/c`, fuse mounts) and
					// for libuv's post-Node 22.17 `EINVAL` on protected Windows
					// paths (see #187).
					if (
						err.code === "ENOENT" ||
						err.code === "EPERM" ||
