...
DirectoryWatcher.prototype.setMissing = function setMissing(itemPath, initial, type) {
	...
	if(this.initialScan) {
		this.initialScanRemoved.add(directoryPath);
	}
	...
	if(itemPath === this.path) { // Is this needed?
		...
		if(!initial && this.watchers[withoutCase(this.path)]) {
			this.watchers[withoutCase(this.path)].forEach(function(w) {
				w.emit("change", itemPath, w.data, initial ? "initial" : type);
			});
		}
		return
	}
	...
	if(oldDirectory) {
		...
		if(this.watchers[withoutCase(itemPath) + "#directory"]) {
			this.watchers[withoutCase(itemPath) + "#directory"].forEach(function(w) {
				w.emit("change", itemPath, w.data, initial ? "initial" : type);
			});
		}
	}
	...
	if(oldFile) {
		...
		if(this.watchers[withoutCase(this.path)]) {
			this.watchers[withoutCase(this.path)].forEach(function(w) {
				w.emit("change", itemPath, null, initial ? "initial" : type);
			});
		}
	}
};
...
DirectoryWatcher.prototype.setDirectory = function setDirectory(directoryPath, initial, type) {
		...
		const checkStats = () => {
			...
			fs.stat(filePath, (err, stats) => {
				...
				if(!stats) {
					...
					this.setDirectory(filePath, false, eventType);
				} else {
					if(stats.mtime) {
...
DirectoryWatcher.prototype.watchMissingOnParent = function watchMissingOnParent(err) {
			...
			fs.stat(itemPath, (err2, stats) => {
				...
				if(stats.isFile()) {
					...
					if(!this.directories[itemPath])
						this.setDirectory(itemPath, true, "initial-scan");
				}
				callback();
			});
		}, () => {
