...
class DirectoryWatcher extends EventEmitter {
	 ...
	 * @private
	 */
	_lstatWithRetry(target, callback) {
		fs.lstat(target, (err, stats) => {
			if (
				...
				this._retryLstatOnBusy(target, BUSY_RETRIES, callback);
				return;
			}
			callback(err, stats);
		});
	 ...
	 * @private
	 */
	_retryLstatOnBusy(target, remaining, callback) {
		setTimeout(() => {
			if (this.closed) return;
			fs.lstat(target, (err, stats) => {
				if (
					...
					this._retryLstatOnBusy(target, remaining - 1, callback);
					return;
				}
				callback(err, stats);
			});
	...
	onWatchEvent(eventType, filename) {
		...
		if (this._activeEvents.get(filename) === undefined) {
			...
			const checkStats = () => {
				if (this.closed) return;
				this._activeEvents.set(filename, false);
				this._lstatWithRetry(target, (err, stats) => {
					if (this.closed) return;
					...
					// Windows AV scanner). _lstatWithRetry already retried before
					// giving up here.
					if (err) {
						if (
							err.code !== "ENOENT" &&
		...
		process.nextTick(() => {
			...
			fs.readdir(this.path, (err, items) => {
				...
				for (const itemPath of itemPaths) {
					this._lstatWithRetry(itemPath, (err2, stats) => {
						if (this.closed) return;
						if (err2) {
							if (
								err2.code === "ENOENT" ||
