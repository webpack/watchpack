...
class DirectoryWatcher extends EventEmitter {
	...
	onWatcherError(err) {
		...
		if (err) {
			...
			// Surface the error to subscribers so callers can react instead of
			// silently treating the directory as removed (fixes #46). Skip
			// watchers without an "error" listener: EventEmitter throws when
			// "error" is emitted with no handler attached, which would be a
			// breaking change for callers that never subscribed.
			for (const set of this.watchers.values()) {
				for (const w of set) {
					if (w.listenerCount("error") > 0) {
						w.emit("error", /** @type {Error | NodeJS.ErrnoException} */ (err));
					}
				}
			}
