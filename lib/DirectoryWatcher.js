...
// Default number of retries and delay between retries (in ms) when an fs
// operation returns EBUSY. EBUSY is transient on Windows when an AV scanner,
// indexer, or another process briefly locks a file; retrying avoids incorrectly
// reporting the file as removed (see webpack/watchpack#223, #44).
const DEFAULT_BUSY_RETRIES = 3;
const DEFAULT_BUSY_RETRY_DELAY = 100;
...
class DirectoryWatcher extends EventEmitter {
	...
	constructor(watcherManager, directoryPath, options = {}) {
		...
		/** @type {number} */
		this.busyRetries =
			typeof options.busyRetries === "number"
				? Math.max(0, options.busyRetries)
				: options.busyRetries === false
					? 0
					: DEFAULT_BUSY_RETRIES;
		/** @type {number} */
		this.busyRetryDelay =
			typeof options.busyRetryDelay === "number" && options.busyRetryDelay >= 0
				? options.busyRetryDelay
				: DEFAULT_BUSY_RETRY_DELAY;
	 ...
	 */
	lstatWithRetry(target, callback) {
		let remaining = this.busyRetries;
		const delay = this.busyRetryDelay;
		const attempt = () => {
			if (this.closed) return;
	...
	onWatchEvent(eventType, filename) {
		...
		if (this._activeEvents.get(filename) === undefined) {
			...
			const checkStats = () => {
				if (this.closed) return;
				this._activeEvents.set(filename, false);
				this.lstatWithRetry(target, (err, stats) => {
					if (this.closed) return;
					...
					if (!stats) {
						...
						if (err && err.code === "EBUSY" && this.busyRetries > 0) {
							return;
						}
		...
		process.nextTick(() => {
			...
			fs.readdir(this.path, (err, items) => {
				...
				for (const itemPath of itemPaths) {
					this.lstatWithRetry(itemPath, (err2, stats) => {
						if (this.closed) return;
						if (err2) {
							if (
								err2.code === "ENOENT" ||
