...
/** @typedef {(string | RegExp)[] | RegExp | string | IgnoredFunction} Ignored */
	...
	if (Array.isArray(ignored)) {
		...
		for (const item of ignored) {
			if (item instanceof RegExp) {
				...
				if (stringRegexp) stringRegexps.push(stringRegexp);
			}
		}
		if (stringRegexps.length === 0 && regexps.length === 0) {
			return () => false;
		}
		if (stringRegexps.length > 0) {
			regexps.push(new RegExp(stringRegexps.join("|")));
		}
		return (item) => {
